...
export function issueOtp(data: AppData, identifier: string) {
  const code = process.env.NODE_ENV === "production" ? String(Math.floor(100000 + Math.random() * 900000)) : "123456";
  if (process.env.NODE_ENV !== "production") {
    console.info(`[FOXSCAN:auth] Generated OTP for ${identifier}: ${code}`);
  }
  const token = `otp.${randomBytes(24).toString("hex")}`;
  data.sessions = data.sessions.filter(s => !s.token.startsWith("otp."));
